  ...
  for (const cell of row.cells || []) {
    ...
    switch (name) {
      ...
      case "Release Notes":
        ...
        break;
      case "Start Date":
        idea.startDate = val || null;
        break;
      case "Target Date":
        idea.targetDate = val || null;
        break;
      default:
        break;
    }
...
export async function createRoadmapItem({
  ...
  if (startDate) cells.push({ columnId: byName["Start Date"], value: startDate });
  if (targetDate) cells.push({ columnId: byName["Target Date"], value: targetDate });
  const data = await ssFetch(`/sheets/${sheetId}/rows`, {
    method: "POST",
    body: JSON.stringify([{ toTop: true, cells }]),
  });
...
export async function updateIdea(rowId, fields) {
  ...
  if (fields.startDate !== undefined) {
    cells.push({ columnId: byName["Start Date"], value: fields.startDate });
  }
  if (fields.targetDate !== undefined) {
    cells.push({ columnId: byName["Target Date"], value: fields.targetDate });
  }
  const data = await ssFetch(`/sheets/${sheetId}/rows`, {
    method: "PUT",
...
export async function deleteIdea(rowId) {
  requireConfig();
  const sheetId = process.env.SMARTSHEET_SHEET_ID;
  await ssFetch(`/sheets/${sheetId}/rows?ids=${rowId}`, { method: "DELETE" });
  return { deleted: true };
}
...
export async function createRoadmapItemsBulk(items) {
  ...
  const rows = items.map((item) => {
    ...
    if (item.startDate) cells.push({ columnId: byName["Start Date"], value: item.startDate });
    if (item.targetDate) cells.push({ columnId: byName["Target Date"], value: item.targetDate });
