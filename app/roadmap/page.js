    ...
    <div className="space-y-6">
      ...
      {quarters.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <div className="flex min-w-max gap-6 border-b border-slate-200 pb-3">
            {quarters.map((q) => (
              <div key={q} className="w-72 flex-shrink-0 text-sm font-bold uppercase tracking-wide text-slate-800">
                {q}
              </div>
            ))}
          </div>

          <div className="flex min-w-max gap-6 pt-4">
            {quarters.map((q) => {
              const items = ideas.filter((i) => i.timeframe === q);
              return (
                <div key={q} className="w-72 flex-shrink-0 space-y-3">
